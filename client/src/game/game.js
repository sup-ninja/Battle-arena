...
export default class Game extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("bullet", playerImage);
  }
  ...
  create() {
    ...
    this.scoreText = this.add
      .text(16, 16, "numbers of kills : " + this.score, {
        font: "18px monospace",
        fill: "#FFFFFF",
        padding: {
          x: 20,
    ...
    this.room.onJoin.add(() => {
      ...
      this.room.onStateChange.addOnce((state) => {
        ...
        for (let id in state.players) {
          ...
          if (self.players[id] == undefined && id != this.room.sessionId) {
      ...
      this.room.state.bullets.onAdd = (bullet, sessionId) => {
        self.bullets[bullet.index] = self.physics.add
          .sprite(bullet.x, bullet.y, "bullet")
    ...
    this.room.onMessage.add((message) => {
      if (message.event == "start_position") {
        let spawnPoint = this.map.findObject(
          "player",
          (obj) => obj.name === `player${message.position}`
        );
        let position = {
          x: spawnPoint.x,
          y: spawnPoint.y,
      ...
      this.input.on(
        ...
        function (pointer) {
          if (!this.shot) {
            ...
            let speed_x =
              Math.cos(this.player.sprite.rotation + Math.PI / 2) * 50;
            let speed_y =
              Math.sin(this.player.sprite.rotation + Math.PI / 2) * 50;
